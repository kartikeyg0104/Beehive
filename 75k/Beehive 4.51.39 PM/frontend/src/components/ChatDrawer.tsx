...
const ChatDrawer: React.FC<ChatDrawerProps> = ({ userId, userRole, targetUserId, onClose }) => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(fetchMessages, 3000); // Reduced interval for better UX
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Drawer */}
        <motion.div 
          className="relative w-full sm:max-w-2xl h-full bg-white dark:bg-gray-900 shadow-2xl flex flex-col sm:flex-row"
          initial={{ x: '100%' }}
          animate={{ x: 0 }}
          exit={{ x: '100%' }}
